...
export const authorizeCreditCard = functions.https.onCall(async (data, context) => {
    ...
    //poe gerar exceção na conexão com a cielo. sem internet por ex.
    try {
...
export const captureCreditCard = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        return {
            ...
            "error": {
                "code": "Erro 1",
                "message": "Problema ao processar pagamento!",
                 "erro": error,
            }
        };
    }
...
//10 > pgto cancelado, 11 > pgto cancelado após 23:59 do dia de autorização
...
//fç para monitorar todos os pedidos e enviar notificação externa para os admins assim q um novo for criado
export const onNewOrder = functions.firestore.document("/orders/{orderId}").onCreate(async (snapshot, context) => {
    const orderId = context.params.orderId;
    //pega lista de todos os admins do sistema
    const querySnapshot = await admin.firestore().collection("admins").get();
//transforma os admins em uma lista 
...
//fç que moitora se houve lateração no status do pedido
